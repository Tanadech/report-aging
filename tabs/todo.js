...
function _renderTodoOverview() {
  ...
  dataUot.forEach(r => {
    ...
    impByDoc[key].totalQty += num(r['จำนวนขอโอน']);
...
function openTodoBranchImpDoc(docNo) {
  ...
  rows.forEach((r, i) => {
    ...
    html += `<tr>
      ...
      <td style="text-align:center;font-size:14px;font-weight:800;color:var(--text);white-space:nowrap;">${fmtN(num(r['จำนวนขอโอน']))}</td>
