...
// เลือกเฉพาะ value เดียว (toggle: ถ้าเลือกอยู่แล้ว → เลือกทั้งหมด)
function setCBOnly(el, value) {
  const { chk } = getCBState(el);
  if (chk.length === 1 && chk[0] === value) {
    checkAllCB(el);
  } else {
    el.querySelectorAll('input[type=checkbox]').forEach(c => { c.checked = c.value === value; });
    if (el._updBtn) el._updBtn();
  }
}
