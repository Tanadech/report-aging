...
function mkChart(id, type, data, opts) {
  ...
  const tickC = L ? '#637381' : '#7b93b0';
  const gridC = L ? 'rgba(145,158,171,.15)' : 'rgba(255,255,255,.06)';
  const dlC   = L ? '#1C252E' : '#e2e8f0';
  const lgC   = L ? '#1C252E' : '#93c5fd';
...
function mkRadialBar(id, labels, values, colors, onClickFilter) {
  ...
  const apex = new ApexCharts(el, {
    ...
    colors: colors.length ? colors : ['#00B8D9'],
    labels,
...
function mkDonut(id, labels, values, colors, onClickFilter) {
  ...
  const apex = new ApexCharts(el, {
    ...
    colors: colors.length ? colors : ['#00B8D9','#22C55E','#fda92d','#FFAB00','#FF5630'],
...
function mkApexBar(id, categories, series, opts = {}) {
  ...
  const colors     = overrideColors || series.map(s => s.color || '#00B8D9');
  const apexSeries = series.map(s => ({ name: s.name, data: s.data }));
  const heightPx   = parseInt(el.parentElement?.style?.height) || 240;
  el.style.height  = heightPx + 'px';

  if (!categories.length || !series.length) {
    el.innerHTML = '<div style="display:flex;align-items:center;justify-content:center;height:100%;color:var(--muted);font-size:13px;">ไม่มีข้อมูล</div>';
    return;
  }
  ...
  const apex = new ApexCharts(el, {
    ...
    legend: hideLegend ? { show: false } : {
      ...
      fontSize: '12px',
      offsetY: 4,
      labels: { colors: isDark ? '#93c5fd' : '#1C252E' },
