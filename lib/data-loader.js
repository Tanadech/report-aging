...
// ── Auto-load จาก data/manifest.json ──
async function autoLoadFromData() {
  try {
    const res = await fetch('data/manifest.json');
    if (!res.ok) throw new Error('ไม่พบ manifest.json');
    const manifest = await res.json();

    const loadXlsx = async (path, label) => {
      const r = await fetch(path);
      if (!r.ok) throw new Error('ไม่พบ ' + path);
      const buf = await r.arrayBuffer();
      const wb  = XLSX.read(buf, { type:'array', cellDates:true, dateNF:'dd/mm/yyyy' });
      const ws  = wb.Sheets[wb.SheetNames[0]];
      return { rows: XLSX.utils.sheet_to_json(ws, { defval:'', raw:false }), name: label };
    };

    const tasks = [];
    if (manifest.out)    tasks.push(loadXlsx('data/' + manifest.out,    manifest.out).then(({ rows, name }) => { if (rows.length) { dataUot = rows; } }));
    if (manifest.in)     tasks.push(loadXlsx('data/' + manifest.in,     manifest.in).then(({ rows, name }) => { if (rows.length) { dataIn  = rows; } }));
    if (manifest.pallet) tasks.push(loadXlsx('data/' + manifest.pallet, manifest.pallet).then(({ rows }) => { if (rows.length) { dataPallet = rows; rebuildPalletMap(); } }));
    if (manifest.car)    tasks.push(loadXlsx('data/' + manifest.car,    manifest.car).then(({ rows }) => { if (rows.length) { dataCar = rows; } }));
    ...
    document.getElementById('meta').textContent = 'โหลดข้อมูลอัตโนมัติจากโฟลเดอร์ data/';
    document.getElementById('alertbox').classList.remove('show');
    rebuild();
    if (dataCar.length) { rebuildCar(); renderCar(); }
  } catch (e) {
    console.warn('autoLoadFromData:', e);
    setStatus('nodata');
  }
}
